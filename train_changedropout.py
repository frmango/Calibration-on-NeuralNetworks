...
def get_model(args, device):
   ...
   # Define a list to hold the models
    models = []
    optimizer = torch.optim.SGD(models[0].parameters(), lr=args.lr, weight_decay=0.005, nesterov=True, momentum=args.momentum)

    if args.model:
        model_dict = model_dict['model_state_dict']
        for droprate in [0, 0.5]:
            model = LeNet5(n_classes=10, droprate=droprate).to(device)
            model.load_state_dict(model_dict)
            models.append(model)
    else:
        for droprate in args.dropout_rates:
            model = LeNet5(n_classes=10, droprate=droprate).to(device)
            models.append(model)
            
    return models, optimizer, model_dict
...
if __name__ == '__main__':
    ...
    models, optimizer, model_dict = get_model(args, device)
    ...
    for model in models:
        ...
        for epoch in range(start_epoch, n_epoch + 1):
            train_loss = train(models, train_loader, criterion, optimizer, device)
            epoch_loss, epoch_acc = validate(models, val_loader, criterion, device)
            # update tensorboard
            if args.tensorboard:
            ...
            # Store test error
            test_errors.append(1 - epoch_acc)
