...
def create_model(args):
    ...
    if args.model == 'GCN':
        return GCN(in_channels= 3, num_classes= 10, num_hidden=num_hidden,
                    drop_rate=0.5, num_layers=num_layers)
    elif args.model == 'GAT':
        # trained on the validation set and validated on the training set for early stopping
        return GAT(in_channels= 3, num_classes = 10, num_hidden=num_hidden,
                    attention_head=attention_head, drop_rate=0.5, num_layers=num_layers)
